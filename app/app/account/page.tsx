import Link from "next/link";

import { SignOutButton } from "./sign-out-button";
import { CheckoutRefresh } from "./checkout-refresh";
import { SubscriptionCta } from "./subscription-cta";
import { getAuthFromServer } from "@/app/lib/clerk";
import { getUserSubscription, isActiveSubscriptionStatus, isPaidPlan, upsertUserSubscription } from "@/app/lib/subscription";

export const dynamic = "force-dynamic";

export default async function AccountPage() {
  const { userId } = await getAuthFromServer();

  let plan: "free" | "pro" = "free";
  let subscriptionStatus = "none";
  let subscriptionError: string | null = null;

  if (userId) {
    try {
      const subscription = await getUserSubscription(userId);
      if (!subscription) {
        await upsertUserSubscription(userId, { plan: "free" });
        plan = "free";
        subscriptionStatus = "none";
      } else {
        plan =
          isPaidPlan(subscription.plan) || isActiveSubscriptionStatus(subscription.stripe_subscription_status)
            ? "pro"
            : "free";
        subscriptionStatus = subscription.stripe_subscription_status ?? "none";
      }
    } catch (error) {
      subscriptionError = error instanceof Error ? error.message : "Failed to load subscription";
      console.error("[account] failed to load subscription", { userId, subscriptionError });
    }
  }

  return (
    <main className="mx-auto w-full max-w-3xl overflow-hidden px-4 py-8 sm:px-6 sm:py-10">
      <h1 className="text-3xl font-bold sm:text-4xl">Account</h1>

      {userId ? (
        <div className="mt-6 space-y-3">
          <CheckoutRefresh initialPlan={plan} />
          <p>You’re signed in.</p>
          <p className="text-sm opacity-80">User ID: {userId}</p>
          <p className="text-sm opacity-80">Plan: {plan === "pro" ? "Pro" : "Free"}</p>
          <p className="text-sm opacity-80">Subscription status: {subscriptionStatus}</p>
          {subscriptionError ? (
            <p className="text-sm text-red-600">Subscription error: {subscriptionError}</p>
          ) : null}

          <div className="mt-4 flex flex-wrap gap-3">
            <Link className="inline-flex min-h-11 items-center rounded-lg border border-current px-4 py-3" href="/pricing">
              Pricing
            </Link>
            <SubscriptionCta plan={plan} />
            <Link className="inline-flex min-h-11 items-center rounded-lg border border-current px-4 py-3" href="/history">
              History
            </Link>
            <SignOutButton />
          </div>
        </div>
      ) : (
        <div className="mt-6 space-y-3">
          <p>You’re signed out.</p>
          <Link className="inline-flex min-h-11 items-center rounded-lg border border-current px-4 py-3" href="/sign-in">
            Sign in
          </Link>
        </div>
      )}
    </main>
  );
}
